...
export default function DatabaseNodeProperties({ formData, onChange }: Props) {
  ...
  const getConnectionStringPlaceholder = () => {
    switch (formData.provider) {
      case "local":
        return "/app/data/local/database.db" // Use local file path instead of JDBC
      case "postgresql":
        return "postgresql://hostname:5432/database" // Use PostgreSQL URL format
      case "mysql":
        return "mysql://hostname:3306/database" // Use MySQL URL format
      case "sqlserver":
        return "mssql://hostname:1433/database" // Use SQL Server URL format
      case "oracle":
        return "oracle://hostname:1521/database" // Use Oracle URL format
      default:
        return "/app/data/local/database.db"
    }
  }
