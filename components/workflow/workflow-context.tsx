...
//               `${baseurl}/clients/${clientId}/file_conversion_configs?dag_id=${dagData.dag_id}`,
//             )
//             if (response.ok) {
...
//             break
//           case "source":
//             output = { data: [{ id: 1, name: "Sample DB Data" }], source: nodeData.table || nodeData.query }
//             break
//           case "database":
    ...
    async (dagData: DAG) => {
      try {
        ...
        addLog({
          ...
          nodeName: "System",
          status: "info",
          message: `Workflow "${dagData.name}" loaded successfully.${dagData.schedule ? ` Schedule: ${dagData.schedule}` : " (Manual execution)"}`,
        })
      } catch (error) {
        console.error("Error loading workflow from DAG:", error)
        toast({
