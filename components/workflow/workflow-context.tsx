...
//         title: "Error",
//         description:
//           "No active workflow to save. Please create a workflow first.",
//         variant: "destructive",
//       });
//       return;
//     }
...
//           case "create-file":
//             console.log(
//               `Simulating CREATE file: ${nodeData.filename} with overwrite: ${nodeData.overwrite}`
//             );
//             output = {
...
//           case "read-file":
//             console.log(
//               `Simulating READ file: ${nodeData.path || nodeData.filename}`
//             );
//             output = {
//               content: `Content of ${
//                 nodeData.path || nodeData.filename || "default.txt"
//               }`,
...
//           case "write-file":
//             console.log(
//               `Simulating WRITE file: ${
//                 nodeData.path || nodeData.filename
//               } with content: ${nodeData.content?.substring(0, 50) || ""}...`
//             );
//             output = {
...
//           case "copy-file":
//             console.log(
//               `Simulating COPY file: ${
//                 nodeData.source_path || nodeData.sourceFilename
//               } to ${
//                 nodeData.destination_path ||
//                 nodeData.targetFilename ||
//                 nodeData.toFilename
//               }`
//             );
//             output = {
...
//           case "rename-file":
//             console.log(
//               `Simulating RENAME file: ${nodeData.oldFilename} to ${nodeData.newFilename}`
//             );
//             output = {
...
//                 nodeData.provider || "unknown"
//               } to ${nodeData.tableName || "unknown"}`
//             );

//             // Check if we have input data from a read-file node
//             if (inputData && (inputData.content || inputData.path)) {
//               console.log(
//                 `Processing file data for database insertion: ${nodeData.tableName}`
//               );
//               output = {
//                 success: true,
//                 rowsProcessed: Math.floor(Math.random() * 1000) + 100,
//                 executionTime: Math.floor(Math.random() * 5000) + 500,
//                 message: `Successfully inserted file data from ${
//                   inputData.path || "uploaded file"
//                 } into ${nodeData.tableName}`,
...
//           case "source":
//             console.log(
//               `Simulating SOURCE operation: ${
//                 nodeData.provider || "unknown"
//               } format ${nodeData.format || "unknown"}`
//             );
//             output = {
...
//           case "code":
//             console.log(
//               `Simulating EXECUTE code: ${nodeData.language || "unknown"}`
//             );
...
//           case "xml-parser":
//             console.log(
//               `Simulating XML PARSE: ${nodeData.xmlString?.substring(0, 50)}...`
//             );
//             output = {
...
//           default:
//             console.warn(
//               `Execution logic not implemented for node type: ${node.type}`
//             );
...
//       (node) => node.type === "start" && node.data?.active !== false
//     );
...
//       await Promise.all(
//         startNodes.map((startNode) => executeNode(startNode.id))
//       );
//       addLog({
//         nodeId: "system",
//         nodeName: "System",
...
//             title: "Error",
//             description:
//               "File conversion workflow requires both input and output paths.",
//             variant: "destructive",
//           });
//           return;
//         }
...
//           !databaseNode.data.tableName
//         ) {
//           toast({
//             title: "Error",
//             description:
//               "Database workflow requires file path, connection string, and table name.",
//             variant: "destructive",
//           });
//           return;
//         }
...
//         );
//         if (!configResponse)
//           throw new Error("Failed to create database workflow config");
//         createdConfigId = configResponse.id;
//       } else {
//         toast({
//           title: "Error",
//           description:
//             "Workflow must contain a recognized operation (read/write files or database).",
//           variant: "destructive",
//         });
//         return;
//       }
...
//           title: "Error",
//           description:
//             "Failed to determine workflow operation or create necessary configuration.",
//           variant: "destructive",
//         });
//         return;
//       }
...
export function WorkflowProvider({ children }: { children: React.ReactNode }) {
  ...
  // Helper function to map database provider to backend-supported provider
  const mapDatabaseProvider = (provider: string): string => {
    // Based on the backend error, let's try simpler provider names
    // or fall back to file-based output for local development
    const providerMapping: Record<string, string> = {
      local: "local", // Keep as local - backend should handle this
      postgresql: "local", // Fallback to local for now
      mysql: "local", // Fallback to local for now
      sqlserver: "local", // Fallback to local for now
      oracle: "local", // Fallback to local for now
    }

    console.log(`Mapping provider ${provider} to ${providerMapping[provider] || "local"}`)
    return providerMapping[provider] || "local"
  }
  ...
  const saveAndRunWorkflow = useCallback(async () => {
    ...
    try {
      ...
      // --- DATABASE WORKFLOW ---
        ...
        // Map the database provider to backend-supported provider
        const mappedProvider = mapDatabaseProvider(databaseNode.data.provider || "postgresql")
        ...
        const configPayload = {
          ...
          output: {
            ...
            format: mappedProvider === "local" ? "parquet" : "sql", // Use parquet for local, sql for databases
            path:
              mappedProvider === "local"
                ? `/app/data/output/${databaseNode.data.tableName || "output"}.parquet`
                : databaseNode.data.connectionString,
            mode: databaseNode.data.writeMode || "overwrite",
            options:
              mappedProvider === "local"
                ...
                : {
                    ...
                    password: databaseNode.data.password || "",
                    batchSize: databaseNode.data.batchSize || "5000",
                    driver: getDatabaseDriver(databaseNode.data.provider || "postgresql"),
                  },
          },
          filter: filterNode
            ? {
