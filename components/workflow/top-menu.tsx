...
import { useWorkflow } from "./workflow-context"; // Assuming TopMenu is in the same directory or adjust path
import { Button } from "@/components/ui/button";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { cn } from "@/lib/utils";
...
import { createClient } from "@/services/client"
import type { ClientCreateResponse } from "@/services/interface"
// import { stopCurrentWorkflow, getCurrentWorkflowId } from "@/services/dagService"; // getCurrentWorkflowId removed
...
export function TopMenu({
  ...
  // Get currentWorkflowId and currentWorkflowName from context as per fix
  const {
    runWorkflow,
    saveWorkflowToBackend,
    saveAndRunWorkflow,
    currentWorkflowName,
    currentWorkflowId // Added from context
  } = useWorkflow();
  ...
  // Dropped old useState and useEffect for workflowIdAvailable
  // Use currentWorkflowId from context directly, as per fix
  const workflowIdAvailable = Boolean(currentWorkflowId);

  const handleCreateClient = async () => {
    if (!clientName.trim()) { setErrorMessage("Client name cannot be empty."); return }
    setIsSubmitting(true); setErrorMessage("")
    try {
      const clientPayload = { name: clientName.trim() }
      const created: ClientCreateResponse | null = await createClient(clientPayload)
      if (!created || !created.id) { throw new Error("Client creation failed or didn't return an ID.") }
      setCreatedClient(created); setClientName("")
      try {
  ...
  const handleStop = async () => {
    // Use currentWorkflowId from context, old dagService.getCurrentWorkflowId() call removed
    if (!currentWorkflowId) {
      setErrorMessage("No active workflow found to stop")
      return
    }
    setIsStopping(true); setErrorMessage("")
    try {
      // stopCurrentWorkflow itself might use localStorage or its own method to get ID
      const result = await stopCurrentWorkflow() 
      if (!result?.success) { setErrorMessage(result?.message || "Failed to stop workflow") }
    } catch (error) { console.error("Error stopping workflow:", error); setErrorMessage("An error occurred while stopping the workflow") }
    ...
    <div className="w-full">
      <div className="flex h-14 items-center justify-between border-b px-4 bg-background">
        <div className="flex items-center space-x-6 h-full">
          {topTabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={cn( "relative pb-1 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors", activeTab === tab && "text-foreground" )}>
              {tab}
        ...
        <div className="flex items-center gap-1 mt-11">
          {/* Render currentWorkflowName from context directly */}
          {currentWorkflowName && (
            <div className="flex items-center gap-2 px-3 py-1 bg-purple-50 rounded-md border border-purple-200">
              <span className="text-sm font-medium text-purple-700">
                {currentWorkflowName}
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Use workflowIdAvailable (derived from context's currentWorkflowId) for disabled state */}
          <Button variant="outline" size="sm" onClick={handleStop}
            disabled={isStopping || !workflowIdAvailable}
            className="text-red-600 hover:text-red-700 hover:border-red-300">
            {isStopping ? <><Loader2 className="h-4 w-4 mr-1 animate-spin" />Stopping...</> : <><Square className="h-4 w-4 mr-1" />Stop</>}
          </Button>
          <Button variant="outline" size="sm" onClick={() => setCreateClientDialogOpen(true)}>
            <UserPlus className="h-4 w-4 mr-1" />Create Client
          </Button>
          <Button variant="outline" size="sm">
      ...
      <Dialog open={createClientDialogOpen} onOpenChange={setCreateClientDialogOpen}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {!createdClient ? (
            ...
            <div className="grid gap-4 py-4">
              <div className="bg-muted p-4 rounded-md">
                <h3 className="font-medium mb-2">Client Created Successfully</h3> <hr />
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <span className="text-muted-foreground">ID:</span><span>{createdClient.id}</span>
                  <span className="text-muted-foreground">Name:</span><span>{createdClient.name}</span>
                </div>
              </div>
            </div>
          )}
