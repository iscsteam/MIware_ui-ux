...
interface SchemaModalProps {
  ...
  baseInputSchema: SchemaItem[];
  baseOutputSchema: SchemaItem[];
  availableInputsFromPrevious: ExtendedSchemaItem[]; // <-- change this
  onClose: () => void;
}
...
const SchemaModal: React.FC<SchemaModalProps> = ({
  ...
  const [selectedCurrentInput, setSelectedCurrentInput] =
    useState<ExtendedSchemaItem | null>(null);
  const [localInputSchema, setLocalInputSchema] = useState<
    ExtendedSchemaItem[]
  >(baseInputSchema as ExtendedSchemaItem[]);

  // availableInputsFromPrevious will also be used as ExtendedSchemaItem
  const groupAvailableInputs = (): GroupedSource[] => {
    const grouped: Record<string, ExtendedSchemaItem[]> = {};

    (availableInputsFromPrevious as ExtendedSchemaItem[]).forEach((source) => {
  ...
  const handleUseSource = (source: ExtendedSchemaItem, targetInput: SchemaItem) => {
    setLocalInputSchema((prev) =>
      prev.map((input) =>
        input.name === targetInput.name
          ? {
              ...input,
              mappedFrom: source.name,
              mappedFromNode: source.sourceNodeLabel,
            }
          : input
      )
    );
        ...
        {localInputSchema.map((item) => {
          ...
          return (
            <div
              ...
              onClick={() =>
                setSelectedCurrentInput(
                  item === selectedCurrentInput ? null : item
                )
              }
                    ...
                    <div key={group.nodeType} className="space-y-1">
                      ...
                      {expandedNodes[group.nodeType] && (
                        <ul className="space-y-1 pl-3">
                          {group.outputs
                            .filter(
                              (output) => output.datatype === item.datatype
                            )
                            .map((source) => (
                              <li
                                key={source.name}
  ...
  const renderOutputSchemaItems = (schemaItems: SchemaItem[]) => {
