...
export function Sidebar() {
  ...
  return (
    <div className="relative h-full">
      <div
        className={`border-r bg-gradient-to-b from-slate-50 to-white flex flex-col h-full shadow-md transition-all duration-300 overflow-hidden ${
          isCollapsed ? "w-16" : "w-64"
        }`}
      >
        {/* Header */}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
                    onClick={() =>
                      !isCollapsed && setIsSampleOpen(!isSampleOpen)
                    }
                    className={cn(
                      "w-full flex items-center justify-between px-4 py-2 text-gray-700 hover:bg-rose-50 hover:text-rose-600 hover:shadow-sm rounded-lg transition-all duration-200 font-semibold",
                      isCollapsed && "justify-center px-2"
