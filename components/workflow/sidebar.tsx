    ...
    <div className="relative h-full">
            ...
            {!isCollapsed && isProjectOpen && (
              <div className="space-y-1">
                ...
                {isWorkflowsOpen && (
                  <div className="pl-14 space-y-1">
                    {isLoadingWorkflows ? (
                      <div className="py-2 text-sm text-gray-500">Loading workflows...</div>
                    ) : workflowError ? (
                      <div className="py-2 text-sm text-red-500">{workflowError}</div>
                    ) : workflows.length === 0 ? (
                      <div className="py-2 text-sm text-gray-500">No workflows found</div>
                    ) : (
                      workflows.map((workflow) => (
                        <Button
                          key={workflow.id}
                          variant="ghost"
                          onClick={() => setActiveView(workflow.id)}
                          className="w-full flex items-center justify-start text-left py-1 text-gray-700 hover:bg-rose-50 hover:text-rose-600 hover:shadow-sm rounded-lg"
                        >
                          <ActivitySquare className="h-4 w-4 text-rose-500 mr-3" />
      ...
      <CreateWorkflowModal
        ...
        onClose={() => {
          ...
          fetchWorkflows() // Refresh workflows after modal closes
        }}
      />
