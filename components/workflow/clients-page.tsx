...
"use client"
import { useState, useEffect } from "react"
import { ArrowLeft, Loader2, RefreshCw, Trash2, Edit, ChevronLeft, ChevronRight, Plus } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { fetchClients, deleteClient, createClient } from "@/services/client"
import type { Client } from "@/services/interface"
...
export function ClientsPage({ onBack }: ClientsPageProps) {
  ...
  const handleCreateClient = async () => {
    if (!clientName.trim()) {
      setCreateErrorMessage("Client name cannot be empty.")
      return
    }
    ...
    try {
      ...
      try {
        ...
        if (workflowDataString) {
          try {
            ...
            console.error("ClientsPage: Failed to parse/update 'currentWorkflow' for new client_id", e)
          }
        }
      } catch (storageError) {
        console.error("ClientsPage: Failed to save client to localStorage:", storageError)
        setCreateErrorMessage(
      ...
      if (error instanceof TypeError && error.message.includes("Failed to fetch")) {
        setCreateErrorMessage("Cannot connect to the API server. Please ensure the backend service is accessible.")
      } else if (error instanceof Error) {
        setCreateErrorMessage(`Error: ${error.message}`)
      } else {
        setCreateErrorMessage("An unknown error occurred.")
      }
    } finally {
  ...
  const filteredClients = clients.filter(
    (client) =>
      client.name.toLowerCase().includes(searchTerm.toLowerCase()) || client.id.toString().includes(searchTerm),
  )
