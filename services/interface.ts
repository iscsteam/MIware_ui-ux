...
// SchemaItem + Mapping Info + Runtime Value
export type MappingSourceInfo = {
  name: string;
  nodeId: string;
  nodeLabel?: string;
...
export type ExtendedSchemaItem = SchemaItem & {
  ...
  mappings?: MappingSourceInfo[];
  value?: any;
  nodeId?: string; // Optional, for runtime context
  
};

// Grouped schema from previous nodes
export interface GroupedSource {
  nodeId: string;
  nodeLabel: string;
  nodeType: string;
...
export interface SchemaModalData {
  ...
  availableInputsFromPrevious: SchemaItem[];
  nodeLabel?: string;
  currentNodeData: WorkflowNodeData; // ← add this

}


// For visual connection lines
export interface Connection {
  sourceItemId: string;
  targetItemId: string;
}

// Modal Props for Schema Configuration
export interface SchemaModalProps {
  nodeId: string;
  nodeType: NodeType;
  nodeLabel?: string;
...
// --- Node Schema Definition (for templates) ---
export interface NodeSchema {
  label: string;
  description: string;
  inputSchema: SchemaItem[];
...
export interface WorkflowNodeData {
  ...
  // Input Mappings
  inputMappings?: Record<string, MappingSourceInfo[]>;

  // File/FileOps related
  filename?: string;
  toFilename?: string;
  sourceFilename?: string;
  targetFilename?: string;
