...
// // Interface for Scheduler sequence
// interface SchedulerSequence {
//   schedulerNode: WorkflowNode
//   sequenceIndex: number
// }

// // Interface for operation config
// interface OperationConfig {
//   type: "file_conversion" | "cli_operator" | "read_salesforce" | "write_salesforce" | "scheduler"
//   configId: number
//   nodeId: string
//   sequenceIndex: number
// }
...
//   salesforceReadConfigs: Map<string, number>
//   salesforceWriteConfigs: Map<string, number>
//   schedulerConfigs: Map<string, number>
// } = {
//   fileConversionConfigs: new Map(),
//   cliOperatorConfigs: new Map(),
//   salesforceReadConfigs: new Map(),
...
// // NEW: Find all Scheduler sequences in the workflow
// function findSchedulerSequences(nodes: WorkflowNode[], connections: NodeConnection[]): SchedulerSequence[] {
//   const sequences: SchedulerSequence[] = []
//   const schedulerNodes = nodes.filter((node) => node.type === "scheduler")
...
// // Find all operations in workflow order (mixed file conversions, CLI operations, Salesforce, and Scheduler)
// function findAllOperationsInOrder(nodes: WorkflowNode[], connections: NodeConnection[]): OperationConfig[] {
//   const operations: OperationConfig[] = []
...
//       currentNode = nextNode
//     }
//     // NEW: Check if current node is a Scheduler operation
//     else if (nextNode.type === "scheduler") {
//       const configId = createdConfigs.schedulerConfigs.get(nextNode.id) || -1
//       operations.push({
//         type: "scheduler",
//         configId,
//         nodeId: nextNode.id,
...
//   } else if (firstConfig.type === "write_salesforce") {
//     firstNodeId = `write_salesforce_${firstConfig.configId}`
//   } else if (firstConfig.type === "scheduler") {
//     firstNodeId = `scheduler_${firstConfig.configId}`
//   } else {
//     firstNodeId = `read_salesforce_${firstConfig.configId}`
//   }
...
//     } else if (config.type === "write_salesforce") {
//       nodeId = `write_salesforce_${config.configId}`
//     } else if (config.type === "scheduler") {
//       nodeId = `scheduler_${config.configId}`
//     } else {
//       nodeId = `read_salesforce_${config.configId}`
//     }
...
//       } else if (nextConfig.type === "write_salesforce") {
//         nextNodeId = [`write_salesforce_${nextConfig.configId}`]
//       } else if (nextConfig.type === "scheduler") {
//         nextNodeId = [`scheduler_${nextConfig.configId}`]
//       } else {
//         nextNodeId = [`read_salesforce_${nextConfig.configId}`]
//       }
//     } else {
...
//   createdConfigs.salesforceReadConfigs.clear()
//   createdConfigs.salesforceWriteConfigs.clear()
//   createdConfigs.schedulerConfigs.clear()
...
//     salesforceRead: createdConfigs.salesforceReadConfigs.size,
//     salesforceWrite: createdConfigs.salesforceWriteConfigs.size,
//     scheduler: createdConfigs.schedulerConfigs.size,
//   }
// }

// // ENHANCED: Create all configurations with support for all conversion types and scheduler
// export async function createAllConfigs(
//   nodes: WorkflowNode[],
//   connections: NodeConnection[],
//   currentWorkflowId: string | null,
...
//   )
//   const hasSalesforce = currentNodeTypes.includes("salesforce-cloud") || currentNodeTypes.includes("write-salesforce")
//   const hasScheduler = currentNodeTypes.includes("scheduler")
...
//     const cliOperationSequences = findCliOperationSequences(nodes, connections)
//     const salesforceSequences = findSalesforceSequences(nodes, connections)
//     const schedulerSequences = findSchedulerSequences(nodes, connections)

//     console.log(`Creating configs for ${fileConversionSequences.length} file conversion sequences`)
//     console.log(`Creating configs for ${cliOperationSequences.length} CLI operation sequences`)
//     console.log(`Creating configs for ${salesforceSequences.length} Salesforce sequences`)
//     console.log(`Creating configs for ${schedulerSequences.length} Scheduler sequences`)
...
//           console.log(`Using fallback CLI config ID ${fallbackConfigId} for ${operationNode.type} operation`)
...
//         }
//       } catch (error) {
//         console.error(`Error creating CLI operator config for ${operationNode.type}:`, error)

//         // Don't fail the entire process, just log and continue with a fallback
//         const fallbackConfigId = Math.floor(Math.random() * 10000)
//         createdConfigs.cliOperatorConfigs.set(operationNode.id, fallbackConfigId)

//         console.log(`Using fallback CLI config ID ${fallbackConfigId} due to error`)
...
//     // NEW: Process Scheduler sequences
//     for (const sequence of schedulerSequences) {
//       const { schedulerNode } = sequence

//       if (!schedulerNode.data.dag_id_to_trigger || !schedulerNode.data.start_time) {
//         toast({
//           title: "Error",
//           description:
//             "Scheduler workflow requires DAG ID to trigger and start time. Please configure the Scheduler node first.",
//           variant: "destructive",
//         })
//         return false
//       }
...
//       try {
//         const timerConfig = mapSchedulerNodeToTimerConfig(schedulerNode)
//         console.log(`Creating scheduler config ${sequence.sequenceIndex + 1}:`, timerConfig)

//         const configResponse = await createSchedulerTimerConfig(clientId, timerConfig)
//         if (!configResponse) {
//           console.warn(
//             `Scheduler config creation returned null for sequence ${sequence.sequenceIndex + 1}, but continuing...`,
//           )

//           // Create a fallback config ID for development
//           const fallbackConfigId = Math.floor(Math.random() * 10000)
//           createdConfigs.schedulerConfigs.set(schedulerNode.id, fallbackConfigId)

//           console.log(
//             `Using fallback scheduler config ID ${fallbackConfigId} for sequence ${sequence.sequenceIndex + 1}`,
//           )

//           toast({
//             title: "Scheduler Config (Development)",
//             description: `Scheduler configuration created with fallback ID ${fallbackConfigId}`,
//             variant: "default",
//           })
//         } else {
//           createdConfigs.schedulerConfigs.set(schedulerNode.id, configResponse.id)

//           console.log(`Created scheduler config ${configResponse.id} for sequence ${sequence.sequenceIndex + 1}`)

//           toast({
//             title: "Scheduler Config Created",
//             description: `Scheduler configuration ${configResponse.id} created successfully!`,
//           })
//         }
//       } catch (error) {
//         console.error("Error creating scheduler config:", error)

//         // Don't fail the entire process, just log and continue with a fallback
//         const fallbackConfigId = Math.floor(Math.random() * 10000)
//         createdConfigs.schedulerConfigs.set(schedulerNode.id, fallbackConfigId)

//         console.log(`Using fallback scheduler config ID ${fallbackConfigId} due to error`)

//         toast({
//           title: "Scheduler Config (Fallback)",
//           description: `Scheduler config created with fallback ID ${fallbackConfigId} due to API error`,
//           variant: "default",
//         })
//       }
//     }
...
//     const salesforceReadCount = salesforceSequences.filter((s) => s.type === "read").length
//     const salesforceWriteCount = salesforceSequences.filter((s) => s.type === "write").length
//     const schedulerCount = schedulerSequences.length

//     toast({
//       title: "Success",
//       description: `✅ Created ${fileConversionCount} file conversion config(s), ${cliOperationCount} CLI operator config(s), ${salesforceReadCount} Salesforce read config(s), ${salesforceWriteCount} Salesforce write config(s), and ${schedulerCount} scheduler config(s).`,
//     })
...
// // ENHANCED: Update all configurations with proper filter normalization and scheduler support
// export async function updateAllConfigs(
//   nodes: WorkflowNode[],
//   connections: NodeConnection[],
//   currentWorkflowId: string | null,
...
//     createdConfigs.salesforceReadConfigs.size === 0 &&
//     createdConfigs.salesforceWriteConfigs.size === 0 &&
//     createdConfigs.schedulerConfigs.size === 0
//   ) {
//     toast({
//       title: "No Configs Found",
//       description: "Please create configs first before updating.",
...
//     console.log(`Updating configs for ${cliOperationSequences.length} CLI operation sequences`)
//     console.log(`Updating configs for ${salesforceSequences.length} Salesforce sequences`)
//     console.log(`Updating configs for ${schedulerSequences.length} Scheduler sequences`)
...
//     // NEW: Update Scheduler sequences
//     for (const sequence of schedulerSequences) {
//       const { schedulerNode } = sequence
//       const configId = createdConfigs.schedulerConfigs.get(schedulerNode.id)

//       if (!configId) {
//         console.log(`No config found for scheduler node ${schedulerNode.id}, skipping update`)
//         continue
//       }

//       try {
//         const timerConfig = mapSchedulerNodeToTimerConfig(schedulerNode)
//         console.log(`Updating scheduler config ${configId}:`, timerConfig)

//         const configResponse = await updateSchedulerTimerConfig(clientId, configId, timerConfig)
//         if (!configResponse) {
//           console.warn(`Scheduler config update returned null for config ${configId}, but continuing...`)

//           toast({
//             title: "Scheduler Config (Development)",
//             description: `Scheduler configuration ${configId} updated in development mode`,
//             variant: "default",
//           })
//         } else {
//           console.log(`Updated scheduler config ${configId} for sequence ${sequence.sequenceIndex + 1}`)

//           toast({
//             title: "Scheduler Config Updated",
//             description: `Scheduler configuration ${configId} updated successfully!`,
//           })
//         }
//       } catch (error) {
...
//         toast({
//           title: "Scheduler Config (Fallback)",
//           description: `Scheduler config ${configId} update completed with fallback handling`,
//           variant: "default",
//         })
//       }
//     }
...
//     toast({
//       title: "Success",
//       description: `Updated ${fileConversionCount} file conversion config(s), ${cliOperationCount} CLI operator config(s), ${salesforceReadCount} Salesforce read config(s), ${salesforceWriteCount} Salesforce write config(s), and ${schedulerCount} scheduler config(s).`,
//     })
...
//   // NEW: Validate scheduler sequences
//   const schedulerSequences = findSchedulerSequences(nodes, connections)
...
//     if (!schedulerNode.data.run_once && (!schedulerNode.data.time_interval || schedulerNode.data.time_interval <= 0)) {
...
//       errors.push(`Scheduler node in sequence ${sequence.sequenceIndex + 1}  {
