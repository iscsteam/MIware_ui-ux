...
export async function saveAndRunWorkflow(
  ...
  try {
    ...
    let operationTypeForDag: "file_conversion" | "cli_operator" | "read_salesforce" | null = null
    ...
    if (salesforceNodes.length > 0) {
      ...
      console.log("Creating Salesforce configuration:", JSON.stringify(configPayload, null, 2))
    ...
    // --- FILE-TO-DATABASE ---
      ...
      if (!readNode.data.path || !databaseNode.data.connectionString || !databaseNode.data.table) {
        toast({
          title: "Error",
          description: "File-to-DB: File path, DB connection, and table required.",
          variant: "destructive",
        })
        return false
      }
    ...
    // --- DATABASESOURCE-TO-FILE ---
      ...
      if (
        ...
        toast({
          title: "Error",
          description: "DB-to-File: DB conn, (query or table name), and output path required.",
          variant: "destructive",
        })
        return false
      }
    ...
    if (operationTypeForDag === "read_salesforce") {
      console.log("Creating Salesforce read config with:", JSON.stringify(configPayload, null, 2))
      const response = await createSalesforceReadConfig(dynamicClientIdString, configPayload)
      if (!response?.id) throw new Error("Failed to create Salesforce read config or ID missing.")
      createdConfigId = response.id
    } else if (operationTypeForDag === "file_conversion") {
      console.log("Creating file_conversion config with:", JSON.stringify(configPayload, null, 2))
      const response = await createFileConversionConfig(clientId, configPayload)
      if (!response?.id) throw new Error("Failed to create file conversion config or ID missing.")
      createdConfigId = response.id
      ...
      if (!response?.id) throw new Error(`Failed to create CLI op config for ${configPayload.operation} or ID missing.`)
      createdConfigId = response.id
    }
    ...
    console.log("Updating DAG with sequence:", JSON.stringify(dagSequence, null, 2))
    ...
    try {
      ...
      if (!triggerResult) console.warn("DAG run trigger returned non-truthy value, but workflow saved.")
    ...
    toast({
      title: "Success",
      description: `${operationTypeForDag === "read_salesforce" ? "Salesforce workflow" : "Workflow"} saved and execution started.`,
    })
    return true
  } catch (error) {
