...
export async function saveAndRunWorkflow(
  ...
  clientId: string | number | null, // Accept string, number, or null
): Promise<boolean> {
  // Cast clientId to a number
  ...
  if (!numericClientId) {
    toast({
      title: "Error",
      description: "No active client ID found. Please select a client first.",
      variant: "destructive",
    })
    return false
  }
  ...
  try {
    ...
    console.log(`saveAndRunWorkflow: Using workflow ID ${workflowId} and client ID ${numericClientId}`)
    ...
    const config = {
      ...
      dag_id: workflowId, // Use the provided workflow ID
    }
    ...
    const configResponse = await createFileConversionConfig(numericClientId, config)
    if (!configResponse) {
    ...
    const updatedDag = await updateDag(workflowId, dagUpdateData)
    if (!updatedDag) {
      throw new Error("Failed to update DAG")
    }

    // 5. Trigger DAG run
    const triggerResult = await triggerDagRun(workflowId)
    if (!triggerResult) {
