...
import {
  ...
  createDatabaseToFileConfig,
  createSalesforceReadConfig,
  updateSalesforceReadConfig,
  mapCopyFileToCliOperator,
  mapMoveFileToCliOperator,
  mapRenameFileToCliOperator,
  mapDeleteFileToCliOperator,
} from "@/services/schema-mapper"
import { createCliOperatorConfig, updateCliOperatorConfig } from "@/services/cli-operator-service"
import { toast } from "@/components/ui/use-toast"
import { getCurrentClientId } from "@/components/workflow/workflow-context"
...
const createdConfigs: {
  ...
  salesforceConfigs: Map<string, number>
} = {
  fileConversionConfigs: new Map(),
  cliOperatorConfigs: new Map(),
  salesforceConfigs: new Map(),
}
...
function findAllOperationsInOrder(nodes: WorkflowNode[], connections: NodeConnection[]): OperationConfig[] {
  ...
  while (currentNode) {
    ...
    // Check if current node is a Salesforce operation
      ...
      const configId = createdConfigs.salesforceConfigs.get(nextNode.id) || -1
      operations.push({
        type: "read_salesforce",
        configId,
        nodeId: nextNode.id,
...
export async function createAllConfigs(
  ...
  try {
    ...
    for (const sequence of cliOperationSequences) {
      ...
      switch (operationNode.type) {
        case "copy-file":
          ...
          cliConfigPayload = mapCopyFileToCliOperator(operationNode, currentWorkflowId)
          break
        case "move-file":
          if (!operationNode.data.source_path || !operationNode.data.destination_path) {
            toast({
          ...
          cliConfigPayload = mapMoveFileToCliOperator(operationNode, currentWorkflowId)
          break
        case "rename-file":
          if (!operationNode.data.source_path || !operationNode.data.destination_path) {
            toast({
          ...
          cliConfigPayload = mapRenameFileToCliOperator(operationNode, currentWorkflowId)
          break
        case "delete-file":
          if (!operationNode.data.source_path) {
            toast({
          ...
          cliConfigPayload = mapDeleteFileToCliOperator(operationNode, currentWorkflowId)
          break
        default:
          throw new Error(`Unsupported CLI operation type: ${operationNode.type}`)
      }
    ...
    for (const sequence of salesforceSequences) {
      ...
      // Store the config ID
      createdConfigs.salesforceConfigs.set(salesforceNode.id, configResponse.id)
    ...
    toast({
      title: "Success",
      description: `Created ${fileConversionCount} file conversion config(s), ${cliOperationCount} CLI operator config(s), and ${salesforceCount} Salesforce config(s).`,
    })
...
export async function updateAllConfigs(
  ...
  if (
    ...
    createdConfigs.salesforceConfigs.size === 0
  ) {
    toast({
  ...
  try {
    ...
    for (const sequence of salesforceSequences) {
      const { salesforceNode } = sequence
      const configId = createdConfigs.salesforceConfigs.get(salesforceNode.id)

      if (!configId) {
        console.log(`No config found for Salesforce node ${salesforceNode.id}, skipping update`)
        continue
      }
      ...
      console.log(`Updating Salesforce config ${configId}:`, configPayload)

      // Update the config
      const configResponse = await updateSalesforceReadConfig(dynamicClientIdString, configId, configPayload)
      if (!configResponse) {
        throw new Error(`Failed to update Salesforce config ${configId}`)
      }

      console.log(`Updated Salesforce config ${configId} for sequence ${sequence.sequenceIndex + 1}`)
    }
    ...
    toast({
      title: "Success",
      description: `Updated ${fileConversionCount} file conversion config(s), ${cliOperationCount} CLI operator config(s), and ${salesforceCount} Salesforce config(s).`,
    })
