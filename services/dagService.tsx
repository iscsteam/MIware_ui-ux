...
export async function getDAGById(dagId: string): Promise<DAG | null> {
  try {
    ...
    if (!res.ok) {
      ...
      console.error(
        `Failed to fetch DAG with ID ${dagId}. Status: ${res.status}. Response: ${errorText}`
      );
      throw new Error(
        `Failed to fetch DAG with ID ${dagId}. Status: ${res.status}`
      );
    }
    return await res.json();
  } catch (error) {
...
export async function checkDAGRunStatus(
  dagId: string,
  triggerId: string
): Promise<DAGStatusResponse | null> {
  try {
    ...
    console.error(
      `Error getting DAG status for dagId=${dagId}, triggerId=${triggerId}:`,
      error
    );
    return null;
  }
 ...
 */
export async function stopActiveDAGRun(
  dagId: string
): Promise<StopDAGResponse | null> {
  try {
    console.log(
      `dagService: Attempting to stop active DAG run for dagId: ${dagId}`
    );
    ...
    if (!res.ok) {
      const errorText = await res.text();
      console.error(
        `Failed to stop DAG run for ${dagId}. Status: ${res.status}. Response: ${errorText}`
      );
    ...
    console.log(
      `dagService: Successfully stopped DAG run for ${dagId}:`,
      response
    );
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : "Unknown error occurred while stopping DAG",
      dag_id: dagId,
    };
  }
}
